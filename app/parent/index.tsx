...
export default function ParentDashboard() {
  ...
  React.useEffect(() => {
    ...
    const upsertPickup = (pickup: any) => {
      console.log("🚀 [PARENT] Upserting pickup:", pickup);
      setPickupRequests((prev) => {
        const idx = prev.findIndex(
          (p) => p.id === pickup.pickupId || p.id === pickup.id
        );
        let updated;
        if (idx !== -1) {
          updated = [...prev];
          updated[idx] = { ...updated[idx], ...pickup };
          console.log("🚀 [PARENT] Updated existing pickup at index:", idx);
        } else {
          updated = [...prev, pickup];
          console.log("🚀 [PARENT] Added new pickup to list");
        }
        // Persist to storage using SocketService
        SocketService.saveParentPickups(updated);
        return updated;
      });
    };
    ...
    const handlePickupRequested = (data: any) => {
      console.log("🚀 [PARENT] Pickup requested event received:", data);
      console.log("🚀 [PARENT] Current parent ID:", parentDetails?.id);

      let pickup;
      try {
        pickup = typeof data === "string" ? JSON.parse(data) : data;
        console.log("🚀 [PARENT] Parsed pickup requested data:", pickup);
      } catch (err) {
        console.error("❌ [PARENT] Parse error (pickup-requested):", err);
        return;
      }

      console.log("🚀 [PARENT] Upserting pickup in state");
      upsertPickup(pickup);
      console.log("🚀 [PARENT] Refreshing pickups to get updated status");
      refreshPickups();
    };
    // pickup-started
    const handlePickupStarted = (data: any) => {
      console.log("🚀 [PARENT] Pickup started event received:", data);

      let pickup;
      try {
        pickup = typeof data === "string" ? JSON.parse(data) : data;
        console.log("🚀 [PARENT] Parsed pickup started data:", pickup);
      } catch (err) {
    ...
    const handleChildPickedUp = (data: any) => {
      console.log("🚀 [PARENT] Child picked up event received:", data);

      let pickup;
      try {
        pickup = typeof data === "string" ? JSON.parse(data) : data;
        console.log("🚀 [PARENT] Parsed child picked up data:", pickup);
      } catch (err) {
    ...
    const handleTripCompleted = (data: any) => {
      console.log("🚀 [PARENT] Trip completed event received:", data);
      
      let pickup;
      try {
        pickup = typeof data === "string" ? JSON.parse(data) : data;
        console.log("🚀 [PARENT] Parsed trip completed data:", pickup);
      } catch (err) {
    ...
    const loadPersistedData = async () => {
      try {
        console.log("🚀 [PARENT] Loading persisted data...");
        const savedPickups = await SocketService.getParentPickups();
        console.log("🚀 [PARENT] Loaded pickups from storage:", savedPickups.length);
        setPickupRequests(savedPickups);
      } catch (error) {
        console.error("🚀 [PARENT] Error loading persisted data:", error);
      }
    };
